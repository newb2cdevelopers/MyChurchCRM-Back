...
import {
  Workfront,
  WorkfrontDocument,
} from 'src/schemas/workfronts/workfront.schema';
...
import {
  ...
  UserAssignment,
  WorkfrontDto,
} from 'src/dtos/workfronts';

@Injectable()
export class WorkfrontProvider {
  constructor(
    @InjectModel(Workfront.name)
    private workfrontModel: Model<WorkfrontDocument>,
  ...
  async getAllWorkfrontsByChurch(churchId: string) {
    return this.workfrontModel.find({ churchId });
  }

  async getAllWorkfrontsByChurchAndModule(churchId: string, moduleId: string) {
    return this.workfrontModel.find({ churchId, moduleId }).sort({ name: 1 });
  }
  ...
  async saveAssignment(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Hubo un error guardando la asignación',
      );
    }
  }

  async getWorkFrontAssignmentData(churchId: string): Promise<GeneralResponse> {
    const frontsByChurch = await this.workfrontModel.find({ churchId });
    const userByChurch = await this.usertModel
      .find({ churchId })
      .populate<{ workfront: Workfront }>('workfront');
