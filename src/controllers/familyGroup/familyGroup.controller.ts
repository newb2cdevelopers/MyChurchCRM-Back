...
export class FamiliyGroupController {
  ...
  async getAllFamilyGroups(@Query() query, @Request() req): Promise< FamilyGroup [] > {
    
    return  await this.familyGroupBusiness.getAllFamilyGroups(query && query.churchId);
  }

  @Post()
  async Create(@Body() familyGroup: FamilyGroup, @Request() req):  Promise<GeneralResponse> {
    return await this.familyGroupBusiness.create(familyGroup);
  }

  @Put()
  async Update(@Body() familyGroup: FamilyGroup, @Request() req):  Promise<GeneralResponse> {
